...
protocol GalleryLoaderDelegate:class  {
    //func successUtf8(data: String)
    func success(parser: ImagesParser,response: String)
    func error(error: Error?,response: String)
}

class  GalleryLoader: MyApi  {
    
    var TAG: String = "Api.swift"
    
    
    var delegate:  GalleryLoaderDelegate? = nil
    
    func setDelegate(context:  GalleryLoaderDelegate) {
        delegate = context
    }
    ...
    var result: ImagesParser? = nil
    
    func parser(json: String) -> ImagesParser {
        
        let mParser = ImagesParser(content: json)
