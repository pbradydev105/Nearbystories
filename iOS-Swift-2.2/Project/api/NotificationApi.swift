...
protocol NotificationLoaderDelegate:class  {
    //func successUtf8(data: String)
    func success(parser: NotificationParser,response: String)
    func error(error: Error?,response: String)
}

class NotificationLoader: MyApi  {
    
    
    var TAG: String = "Api.swift"
   
    var delegate: NotificationLoaderDelegate? = nil
    
    func setDelegate(context: NotificationLoaderDelegate) {
        delegate = context
    }
    ...
    var result: NotificationParser? = nil
    
    func parser(json: String) ->  NotificationParser {
        
        let mParser = NotificationParser(content: json)
