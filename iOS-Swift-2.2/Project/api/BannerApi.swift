...
protocol BannerLoaderDelegate:class  {
    //func successUtf8(data: String)
    func success(parser: BannerParser,response: String)
    func error(error: Error?,response: String)
}

class BannerLoader: MyApi  {
    
    
    var TAG: String = "Api.swift"
   
    var delegate: BannerLoaderDelegate? = nil
    
    func setDelegate(context: BannerLoaderDelegate) {
        delegate = context
    }
    ...
    var result: BannerParser? = nil
    
    func parser(json: String) ->  BannerParser {
        
        let mParser = BannerParser(content: json)
